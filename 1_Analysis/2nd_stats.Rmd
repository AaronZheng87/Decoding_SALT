...
c_valence1 <- readr::read_csv(here("2_Data", "result1_cross_valence1.csv"))
c_valence2 <- readr::read_csv(here("2_Data", "result1_cross_valence2.csv"))
```
```{r}
avg_fold_c1 <- c_con1 %>%
...
avg_fold_cv1 <- c_valence1 %>%
  dplyr::group_by(sub, source, target) %>%
  dplyr::summarise(m_score = mean(score)) %>%
  ungroup()

avg_fold_cv2 <- c_valence2 %>%
  dplyr::group_by(sub, source, target) %>%
  dplyr::summarise(m_score = mean(score)) %>%
  ungroup()
...
nest_st_v1 <- avg_fold_cv1 %>%
  dplyr::group_nest(source, target)

nest_st_v2 <- avg_fold_cv2 %>%
  dplyr::group_nest(source, target)
...
result_vall <- rbind(dat_v1, dat_v2)
result_vall <- result_vall %>%
  dplyr::rename(
    mean_roc_auc_score = mean,
    p_value = p_val
  ) %>%
...
write_csv(result_vall, paste(here("2_Data"), "permutation_result_cro_valence.csv", sep = "/"))
